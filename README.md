...
---
## 👋 자기소개
안녕하세요. 저는 Unity와 C#을 기반으로 실시간 3D 콘텐츠와 인터랙티브 시스템을 개발해 온 최민준입니다.

4년간 다양한 Unity 프로젝트를 진행하며 사용자 입력 처리, 객체 상태 관리, AI 제어, UI, 물리 상호작용 등 실시간 3D 환경에서 필요한 기능들을 직접 구현해 보았습니다. 프로젝트를 진행하는 과정에서 단순히 기능을 완성하는 것뿐 아니라, 이후 기능 추가와 수정이 쉽도록 구조를 나누고 정리하는 방법에 대해 꾸준히 고민해 왔습니다.

특히 장기간 진행한 3D 프로젝트를 통해 입력, 상태, AI, 애니메이션, UI가 서로 연결되는 구조를 경험했으며, 이후에는 Gemini API, MusicGen Server, Unity ML-Agents 등을 활용해 AI 기반 인터랙션과 시뮬레이션 프로젝트도 진행했습니다.

이러한 프로젝트 경험을 바탕으로 Unity의 다양한 기능과 기술을 꾸준히 익히며, 새로운 환경에서도 빠르게 학습하고 성장할 수 있는 개발자가 되고자 합니다.
...
**📝 설명**: 3년간 진행한 Unity 기반 3D 프로젝트로, 캐릭터 조작부터 전투, AI, 아이템, UI까지 게임을 구성하는 다양한 기능을 직접 구현해 보았습니다.<br>
캐릭터 이동과 근·원거리 무기 전환, 애니메이션, FSM 및 NavMesh 기반 적 AI, 상태 변화 시스템 등을 구현하며 여러 기능이 하나의 흐름으로 연결되는 구조를 경험했습니다.<br>
기능이 점차 늘어나는 과정에서 코드 구조를 나누고 모듈화하는 방식을 적용해 보며, 유지보수와 확장성을 고려한 개발 방식에 대해 학습했습니다.<br>
...
**📝 설명**: 사용자의 입력을 기반으로 AI가 NPC 대사를 생성하고, 대화의 분위기에 따라 BGM을 생성·재생하는 Unity 기반 AI 인터랙션 프로젝트를 진행했습니다.<br>
Gemini API와 MusicGen Server를 Unity와 연동하고 Coroutine을 활용해 대화 생성과 음악 재생이 순차적으로 이어지도록 구현했습니다.<br>
이 프로젝트를 통해 외부 API를 Unity 환경에 연결하고, AI의 결과를 실제 콘텐츠에 반영하는 기본적인 시스템 통합 과정을 경험했습니다.<br>
...
또한 처음 개발을 접하는 팀원에게 구현 방식을 설명하고 코드 이해를 도우며, 다른 사람에게 개발 내용을 전달하고 함께 문제를 해결하는 협업 경험을 쌓았습니다.<br>
...
GitHub를 통한 버전 관리와 코드 피드백을 경험했으며, 팀원들과 기능을 나누어 개발하고 발생한 문제를 함께 해결하는 과정을 경험했습니다. <br>
...
학습 환경에서 관찰–행동–보상 흐름을 직접 구성했으며, 목표 탐지/접근/충돌/시간 제한 등 상황별 보상 구조를 적용해 안정적인 학습이 가능하도록 설계했습니다.<br>
